...
use crate::metal_runtime::MetalError;
...
pub type MetalResult<T> = crate::metal_runtime::MetalResult<T>;
...
impl<B: StaticAllocationBackend> InnerStaticAllocator<B> {
    ...
    pub(crate) fn alloc<T>(
    ...
    ) -> MetalResult<StaticAllocationData<T>> {
        ...
        match self.tracker.alloc(alloc_len, placement) {
            ...
            Err(_) => Err(MetalError::Other("Memory allocation failed".into())),
        }
    }
...
impl<B: StaticAllocationBackend, W: InnerStaticAllocatorWrapper<B>> StaticAllocator<B, W> {
    ...
    pub fn alloc<T>(
        ...
        len: usize,
        placement: AllocationPlacement,
    ) -> MetalResult<StaticAllocation<T, B, W>> {
        self.inner
