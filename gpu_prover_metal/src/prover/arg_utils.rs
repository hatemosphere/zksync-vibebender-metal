//! Argument structures shared between stage 2 and stage 3 kernel dispatch.
//! These must match the layout of corresponding MSL structs in arg_utils.metal.
...
impl FlattenedLookupExpressionsLayout {
    pub fn new<F: Fn(usize) -> usize>(
        ...
        let mut coeffs = [0u32; MAX_EXPRESSION_TERMS];
        let mut col_idxs = [0u16; MAX_EXPRESSION_TERMS];
        let mut constant_terms = [BF::ZERO; MAX_EXPRESSIONS];
        let mut num_terms_per_expression = [0u8; MAX_EXPRESSIONS];
        let mut bf_dst_cols = [0u8; MAX_EXPRESSION_PAIRS];
        let mut e4_dst_cols = [0u8; MAX_EXPRESSION_PAIRS];
...
impl FlattenedLookupExpressionsForShuffleRamLayout {
    pub fn new<F: Fn(usize) -> usize>(
        ...
        let mut coeffs = [0u32; MAX_EXPRESSION_TERMS_FOR_SHUFFLE_RAM];
        let mut col_idxs = [0u16; MAX_EXPRESSION_TERMS_FOR_SHUFFLE_RAM];
        let mut constant_terms = [BF::ZERO; MAX_EXPRESSIONS_FOR_SHUFFLE_RAM];
        let mut num_terms_per_expression = [0u8; MAX_EXPRESSIONS_FOR_SHUFFLE_RAM];
        let mut bf_dst_cols = [0u8; MAX_EXPRESSION_PAIRS_FOR_SHUFFLE_RAM];
        let mut e4_dst_cols = [0u8; MAX_EXPRESSION_PAIRS_FOR_SHUFFLE_RAM];
        let mut expression_idx: usize = 0;
        let mut flat_term_idx: usize = 0;
        let mut stash_expr = |expr: &LookupExpression<BF>,
                              expression_idx: &mut usize,
            ...
            for (coeff, column_address) in a.linear_terms.iter() {
                ...
                col_idxs[*flat_term_idx] = match column_address {
                    ...
                    _ => panic!(
                        "Shuffle-ram expressions are expected to use witness, memory, or setup cols only",
                    ),
                };
                *flat_term_idx += 1;
            }
...
#[derive(Clone)]
#[repr(C)]
pub struct MultiplicitiesLayout {
    pub src_cols_start: u32,
...
#[derive(Clone)]
#[repr(C)]
pub struct StateLinkageConstraints {
...
pub struct BoundaryConstraints {
    ...
    pub num_first_row: u32,
    pub num_one_before_last_row: u32,
}

#[derive(Clone, Default)]
#[repr(C)]
pub struct ConstantsTimesChallenges {
    pub first_row: E4,
    pub one_before_last_row: E4,
    pub sum: E4,
}

// Width 3 lookups layout constants (must match stage_3.cu / stage_3.metal)
...
#[derive(Clone)]
#[repr(C)]
pub struct NonDelegatedWidth3LookupsLayout {
    pub coeffs: [u32; NON_DELEGATED_MAX_WIDTH_3_LOOKUP_COEFFS],
    pub col_idxs: [u16; NON_DELEGATED_MAX_WIDTH_3_LOOKUP_COLS],
    pub num_terms_per_expression: [u8; NON_DELEGATED_MAX_WIDTH_3_LOOKUP_VALS],
    pub table_id_is_col: [bool; NON_DELEGATED_MAX_WIDTH_3_LOOKUPS],
    pub e4_arg_cols: [u16; NON_DELEGATED_MAX_WIDTH_3_LOOKUPS],
...
#[derive(Clone)]
#[repr(C)]
pub struct DelegatedWidth3LookupsLayout {
    pub coeffs: [u32; DELEGATED_MAX_WIDTH_3_LOOKUP_COEFFS],
    pub col_idxs: [u16; DELEGATED_MAX_WIDTH_3_LOOKUP_COLS],
    pub num_terms_per_expression: [u8; DELEGATED_MAX_WIDTH_3_LOOKUP_VALS],
    pub table_id_is_col: [bool; DELEGATED_MAX_WIDTH_3_LOOKUPS],
    pub e4_arg_cols: [u16; DELEGATED_MAX_WIDTH_3_LOOKUPS],
    pub helpers_offset: u32,
    pub num_helpers_used: u32,
    pub num_lookups: u32,
    pub e4_arg_cols_start: u32,
...
impl Default for DelegatedWidth3LookupsLayout {
    fn default() -> Self {
        Self {
            coeffs: [0; DELEGATED_MAX_WIDTH_3_LOOKUP_COEFFS],
            col_idxs: [0; DELEGATED_MAX_WIDTH_3_LOOKUP_COLS],
            num_terms_per_expression: [0; DELEGATED_MAX_WIDTH_3_LOOKUP_VALS],
...
pub fn get_grand_product_col(
    circuit: &cs::one_row_compiler::CompiledCircuitArtifact<BF>,
    cached_data: &prover::prover_stages::cached_data::ProverCachedData,
) -> usize {
