...
use crate::allocator::host::{ConcurrentStaticHostAllocator, HostAllocation};
...
impl<K: Clone + Debug + Eq + Hash> ExecutionProver<K> {
    pub fn new(
        ...
        // On Apple Silicon, there's always 1 Metal GPU device
        ...
        // On Metal, use page-aligned host allocations instead of CUDA pinned memory
        let allocations: Vec<HostAllocation> = (0..total_allocators_count)
            .into_par_iter()
            .map(|_| HostAllocation::alloc(length))
            .collect();
    ...
    pub fn commit_memory_and_prove(
        ...
        let caps = &self.binaries[&binary_key]
            ...
            .unwrap()
            .caps;
        log::info!("fs_transform: setup_caps={}, reg_values={}, main_mem={}, deleg_mem={}",
            caps.len(), final_register_values.len(), main_memory_commitments.len(),
            delegation_memory_commitments.len());
        // Log first cap hash for comparison
        if !caps.is_empty() {
            log::info!("fs_transform: setup_cap[0][0] = {:?}", &caps[0].cap[0][..4]);
        }
        if !main_memory_commitments.is_empty() {
            log::info!("fs_transform: main_mem[0][0][0] = {:?}", &main_memory_commitments[0][0].cap[0][..4]);
        }
        for (t, caps) in delegation_memory_commitments.iter() {
            if !caps.is_empty() && !caps[0].is_empty() {
                log::info!("fs_transform: deleg_mem type={}, [0][0][0] = {:?}", t, &caps[0][0].cap[0][..4]);
            }
        }
        ...
        log::info!("fs_transform: seed = {:?}", memory_challenges_seed);
        let produce_delegation_challenge = self.binaries[&binary_key]
            .circuit_type
            .needs_delegation_challenge();
        log::info!("fs_transform: produce_delegation_challenge={}", produce_delegation_challenge);
...
impl<K: Debug + Eq + Hash> Drop for ExecutionProver<K> {
    fn drop(&mut self) {
