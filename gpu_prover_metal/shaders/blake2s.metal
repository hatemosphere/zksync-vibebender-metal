...
DEVICE_FORCEINLINE void compress(thread u32 state[STATE_SIZE], thread u32 &t,
  ...
  G(0,4,8,12,m[14],m[10]) G(1,5,9,13,m[4],m[8]) G(2,6,10,14,m[9],m[15]) G(3,7,11,15,m[13],m[6])
  G(0,5,10,15,m[1],m[12]) G(1,6,11,12,m[0],m[2]) G(2,7,8,13,m[11],m[7]) G(3,4,9,14,m[5],m[3])
  // Round 2: sigma = {11,8,12,0,5,2,15,13,10,14,3,6,7,1,9,4}
  G(0,4,8,12,m[11],m[8]) G(1,5,9,13,m[12],m[0]) G(2,6,10,14,m[5],m[2]) G(3,7,11,15,m[15],m[13])
  G(0,5,10,15,m[10],m[14]) G(1,6,11,12,m[3],m[6]) G(2,7,8,13,m[7],m[1]) G(3,4,9,14,m[9],m[4])
  // Round 3: sigma = {7,9,3,1,13,12,11,14,2,6,5,10,4,0,15,8}
  G(0,4,8,12,m[7],m[9]) G(1,5,9,13,m[3],m[1]) G(2,6,10,14,m[13],m[12]) G(3,7,11,15,m[11],m[14])
  G(0,5,10,15,m[2],m[6]) G(1,6,11,12,m[5],m[10]) G(2,7,8,13,m[4],m[0]) G(3,4,9,14,m[15],m[8])
  // Round 4: sigma = {9,0,5,7,2,4,10,15,14,1,11,12,6,8,3,13}
  G(0,4,8,12,m[9],m[0]) G(1,5,9,13,m[5],m[7]) G(2,6,10,14,m[2],m[4]) G(3,7,11,15,m[10],m[15])
  G(0,5,10,15,m[14],m[1]) G(1,6,11,12,m[11],m[12]) G(2,7,8,13,m[6],m[8]) G(3,4,9,14,m[3],m[13])
  // Round 5: sigma = {2,12,6,10,0,11,8,3,4,13,7,5,15,14,1,9}
  G(0,4,8,12,m[2],m[12]) G(1,5,9,13,m[6],m[10]) G(2,6,10,14,m[0],m[11]) G(3,7,11,15,m[8],m[3])
  G(0,5,10,15,m[4],m[13]) G(1,6,11,12,m[7],m[5]) G(2,7,8,13,m[15],m[14]) G(3,4,9,14,m[1],m[9])
  // Round 6: sigma = {12,5,1,15,14,13,4,10,0,7,6,3,9,2,8,11}
  G(0,4,8,12,m[12],m[5]) G(1,5,9,13,m[1],m[15]) G(2,6,10,14,m[14],m[13]) G(3,7,11,15,m[4],m[10])
  G(0,5,10,15,m[0],m[7]) G(1,6,11,12,m[6],m[3]) G(2,7,8,13,m[9],m[2]) G(3,4,9,14,m[8],m[11])
  #pragma unroll
  for (unsigned i = 0; i < STATE_SIZE; ++i)
