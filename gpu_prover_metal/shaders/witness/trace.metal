...
struct TimestampData {
  static constant constexpr uint NUM_TIMESTAMP_DATA_LIMBS = 3;
  static constant constexpr uint TIMESTAMP_COLUMNS_NUM_BITS = 19;
  static constant constexpr uint TIMESTAMP_COLUMNS_NUM_BITS_MASK = (1u << TIMESTAMP_COLUMNS_NUM_BITS) - 1;
  static constant constexpr uint NUM_EMPTY_BITS_FOR_RAM_TIMESTAMP = 2;

  u16 limbs[NUM_TIMESTAMP_DATA_LIMBS];

  DEVICE_FORCEINLINE TimestampScalar as_scalar() const {
    TimestampScalar result = limbs[0];
    for (int i = 1; i < (int)NUM_TIMESTAMP_DATA_LIMBS; ++i)
...
struct RegIndexOrMemWordIndex {
  static constant constexpr u32 IS_RAM_MASK = 0x80000000;
  u32 value;
