...
template <const bf *fine_powers> DEVICE_FORCEINLINE bf get_cmem_smem_twiddle(const int i, const bf *coarse_powers) {
  int fine_idx = (i >> GMEM_COARSE_LOG_COUNT) & CMEM_FINEST_MASK;
  int coarse_idx = i & GMEM_COARSE_MASK;
...
#pragma unroll
  for (int region{0}; region < NUM_REGIONS; region++) {
    const bf twiddle = get_cmem_smem_twiddle<ab_inv_cmem_twiddles_finest>(exchg_region_offset + region, smem_twiddles);
...
#pragma unroll
  for (int region{0}; region < NUM_REGIONS; region++) {
    const bf twiddle = get_cmem_smem_twiddle<ab_fwd_cmem_twiddles_fine>(exchg_region_offset + region, smem_twiddles);
    const int region_offset = region * REGION_SIZE;
#pragma unroll
    for (int lane_in_region{0}; lane_in_region < STRIDE; lane_in_region++) {
      const int i = region_offset + lane_in_region;
...
#pragma unroll 1
    for (int ntt_idx{0}; ntt_idx < num_ntts - 1; ntt_idx++) {
        ...
        // while(!__mbarrier_try_wait(&bars[0], token, 1000)) {} // fails to compile on sm89, error says it requires sm90+
...
#pragma unroll 4 // helps avoid register spilling, should be fine because prefetches are fire and forget
        ...
        // while(!__mbarrier_try_wait(&bars[1], token, 1000)) {} // fails to compile on sm89, error says it requires sm90+
...
#pragma unroll
    ...
    reg_exchg_cmem_smem_twiddles_inv<16, 32, 1>(vals, thread_exchg_region_offset, smem_twiddles); thread_exchg_region_offset <<= 1;
    reg_exchg_cmem_smem_twiddles_inv<8, 16, 2>(vals, thread_exchg_region_offset, smem_twiddles); thread_exchg_region_offset <<= 1;
    reg_exchg_cmem_smem_twiddles_inv<4, 8, 4>(vals, thread_exchg_region_offset, smem_twiddles); thread_exchg_region_offset <<= 1;
    reg_exchg_cmem_smem_twiddles_inv<2, 4, 8>(vals, thread_exchg_region_offset, smem_twiddles); thread_exchg_region_offset <<= 1;
    reg_exchg_cmem_smem_twiddles_inv<1, 2, 16>(vals, thread_exchg_region_offset, smem_twiddles);
